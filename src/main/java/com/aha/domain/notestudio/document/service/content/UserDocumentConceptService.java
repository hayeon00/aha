...
import com.aha.domain.notestudio.document.client.content.AiConceptGenerationClient;
...
public class UserDocumentConceptService {
    ...
    public List<OwnedDocumentResponseDto> getOwnedDocuments(Long userId, Long userExamId) {
        ...
        List<com.aha.domain.notestudio.document.entity.DocumentProcessingGroup> sessions =
                processingGroupRepository.findCurrentSessions(userExamId, userId);
        if (sessions.isEmpty()) {
            return List.of();
        }
        return documentRepository
                .findAllByProcessingGroup_IdOrderByIdAsc(sessions.get(0).getId())
    ...
    public DocumentConceptDashboardResponseDto getDashboard(Long userId, Long documentId) {
        ...
        Map<Long, String> documentContents = learningNoteContentRepository
                .findAllByLearningNote_ProcessingGroup_IdOrderByExamScopeNode_DisplayOrderAsc(
                        document.getProcessingGroup().getId())
